...
* _[Expressive power](https://en.wikipedia.org/wiki/Expressive_power_(computer_science))_ - The breadth of patterns that can be expressed. While Turing-complete imperative language may be used to describe how to match _any_ computable pattern, the expressive power of pure declarative languages is usually limited (there would be computable patterns that cannot be expressed in such language). One cannot expect to be able to describe any pattern without describing computational steps. At least not readily and concisely.
