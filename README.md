...
An [_undirected graph_](https://en.wikipedia.org/wiki/Graph_(discrete_mathematics)) is an ordered triple 𝐺 = (𝑉, 𝐸, _ψₑ_) consisting of two pairwise disjoint sets and one function, a [_directed graph_](https://en.wikipedia.org/wiki/Directed_graph) (_digraph_, _oriented graph_) is an ordered triple 𝐺 = (𝑉, 𝐴, _ψₐ_) consisting of two pairwise disjoint sets and one function, and a [_mixed graph_](https://en.wikipedia.org/wiki/Mixed_graph) is an ordered quintuple 𝐺 = (𝑉, 𝐸, 𝐴, _ψₑ_, _ψₐ_) consisting of three pairwise disjoint sets and two functions, where 𝑉 is a nonempty set whose elements are called [_vertices_](https://en.wikipedia.org/wiki/Vertex_(graph_theory)) (_nodes_, _dots_, _points_), 𝐸 is a set whose elements are called _undirected edges_ (_undirected links_, _undirected lines_), _ψₑ: E → { {u,v}: u,v ∈ V }_ is a total function mapping each undirected edge to an unordered pair of vertices, 𝐴 is a set whose elements are called _directed edges_ (_directed links_, _directed lines_, _arcs_, _arrows_), and _ψₐ: A → { (u,v): u,v ∈ V }_ is a total function mapping each directed edge to an ordered pair of vertices. _ψₑ_ and _ψₐ_ are called _incidence functions_.

Given undirected edge 𝑒: _ψₑ_(𝑒) = {𝑢,𝑣}, we say that 𝑒 is an edge _between_ 𝑢 and 𝑣, 𝑒 _connects_ (_joins_) 𝑢 and 𝑣, 𝑒 is _incident to_ 𝑢 and 𝑣, 𝑢 and 𝑣 are _connected_ (_adjacent_, _neighbors_), 𝑢 and 𝑣 are the _ends of_ 𝑒, and 𝑢 and 𝑣 are _incident to_ 𝑒. Likewise, given directed edge 𝑎: _ψₐ_(𝑎) = (𝑢,𝑣), we say that 𝑎 is an edge _from_ 𝑢 _to_ 𝑣, 𝑎 _connects_ (_joins_) 𝑢 to 𝑣, 𝑎 is an _outgoing edge_ of 𝑢, 𝑎 is _incident from_ 𝑢, 𝑎 is an _incoming edge_ of 𝑣, 𝑎 is _incident to_ 𝑣, 𝑣 is _adjacent from_ (_out-adjacent to_, _direct successor of_) 𝑢, 𝑢 is _adjacent to_ (_in-adjacent to_, _direct predecessor of_) 𝑣, 𝑢 is the _tail_ (_source_, _initial vertex_) of 𝑎, 𝑢 is _incident to_ 𝑎, 𝑣 is the _head_ (_target_, _terminal vertex_) of 𝑎, and 𝑣 is _incident from_ 𝑎.
...
Extending the _mixed graph_ definition, a _property graph_ is a septet 𝐺 = (𝑉, 𝐸, 𝐴, _ψₑ_, _ψₐ_, _λ_, _σ_) where _λ_: 𝑉 ∪ 𝐸 ∪ 𝐴 → 𝐿 is a total function mapping each node and edge to a label, and _σ_: 𝑉 ∪ 𝐸 ∪ 𝐴 → 2^(𝑃ₙ × 𝑃ᵥ) is a total function mapping each node and edge to a set of properties.
