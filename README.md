...
Given undirected edge 𝑒: _ψₑ_(𝑒) = {𝑢,𝑣}, we say that 𝑒 is an edge _between_ 𝑢 and 𝑣, 𝑒 _connects_ (_joins_) 𝑢 and 𝑣, 𝑒 is _incident to_ 𝑢 and 𝑣, 𝑢 and 𝑣 are _connected_ (_adjacent_, _neighbors_), and 𝑢 and 𝑣 are _ends of_ (_incident to_ 𝑒) 𝑒. Likewise, given directed edge 𝑎: _ψₐ_(𝑎) = (𝑢,𝑣), we say that 𝑎 is an edge _from_ 𝑢 _to_ 𝑣, 𝑎 _connects_ (_joins_) 𝑢 to 𝑣, 𝑎 is an _outgoing edge_ of 𝑢, 𝑎 is _incident from_ 𝑢, 𝑎 is an _incoming edge_ of 𝑣, 𝑎 is _incident to_ 𝑣, 𝑣 is _adjacent from_ (_out-adjacent to_, _direct successor of_) 𝑢, 𝑢 is _adjacent to_ (_in-adjacent to_, _direct predecessor of_) 𝑣, 𝑢 is the _tail of_ (_source of_, _initial vertex of_, _incident to_) 𝑎, and 𝑣 is _head of_ (_target of_, _terminal vertex of_, _incident from_) 𝑎.
