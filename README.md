...
- _Null-valued_ [sub]property indicates that a [sub]property value is missing.
  ...
  - _Applicable missing_ – the value at present is applicable but unknown. E.g., a value that was not [yet] measured or collected; a phone number of a person who owns a phone but the number is unknown; a question's answer – if the questionee refused to answer.
  - _Inapplicable missing_ - the value at present is inapplicable. E.g., previous citizenship when there is none; direct manager of the CEO; new hire's not-yet-assigned employee ID; time of an event that did not happen [yet]; a phone number of a person who does not own a phone; a question's answer – when the question was not presented to the questionee.
  ...
  - _No-information missing_ – it is unknown whether the unknown value is applicable or not. E.g., a person's phone number – if it is unknown whether the person owns a phone; a question's answer – when it is unknown if the question was presented to the questionee.
  ...
  Codd, Zaniolo, and others suggested using two or more types of _null_ values instead of a 'generic' _null_, but this approach remains mainly theoretical. In practice, _null_ values often have no consistent semantics. Sometimes, _null_ values represent applicable missing values, while _magic values_ (e.g., '9999-12-31' for dates) represent inapplicable values. 
  
  Though _null_ value semantics is usually not defined as part of the data model, the semantics of operators and functions is well-defined for _null_ values. E.g., what is the result of (yesterday's date < person's death date) when the death date is _null_?
