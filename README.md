...
- Each entity, each relationship, and each action has a single non-distinct type (string). Types can be assigned based on many universals (qualities), e.g., _person_ entities, _red_ entities, _owner_ entities. Many times types are defined in accordance with the following guidelines:
