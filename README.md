...
_Multivalued properties_ (ordered or unordered, with or without duplicates) and _composite properties_ are sometimes supported as well. Composite properties are composed of a set of sub-properties, each has a name and a value of a certain data type. For example: _name_ = (_first_: _string_ = "Brandon", _last_: _string_ = "Stark"). 
...
* _Declarative / Imperative_ - _[Declarative](https://en.wikipedia.org/wiki/Declarative_programming)_ languages describe patterns but do not necessarily specify how to match them. _[Imperative](https://en.wikipedia.org/wiki/Imperative_programming)_ languages describe patterns in terms of the steps required to match them on a given computation model. Some languages provide both declarative and imperative constructs
...
| _min_(_St, n_) → _St_ <br> _max_(_St, n_) → _St_ | _t_ is an ordinal type <br> set of (up to) _n_ smallest/largest values <br> _null_ when _St_ is _null_, {} when it contains no _non-null_ elements 
...
Implementations may support additional data types, operators, functions, and constraints. Implementations may support _[opaque data types](https://en.wikipedia.org/wiki/Opaque_data_type)_ - data types where the internal data representation is not exposed. For each opaque data type - a set of functions and operators may be defined (see _location_ data type in [Application: Spatiotemporality](#application-spatiotemporality)).
...
* There are at least ten assignments to B such that (B froze A, and A froze a dragon that is not B)
